...
class MetricsService {
    ...
    riskScore;
    // Histograms
    tradeProfitHistogram;
    executionTimeHistogram;
    gasUsedHistogram;
    priceImpactHistogram;
    simulationTimeHistogram;
    constructor() {
    ...
    initializeMetrics() {
        ...
        // Gauges
        this.currentPrice = new prom_client_1.Gauge({
        ...
        this.exposureByToken = new prom_client_1.Gauge({
            name: 'arb_exposure_usd',
            help: 'Current exposure in USD by token',
            labelNames: ['token'],
            registers: [this.register],
        });
        this.circuitBreakerStatus = new prom_client_1.Gauge({
            name: 'arb_circuit_breaker_status',
            help: 'Circuit breaker status (1 = active, 0 = inactive)',
            registers: [this.register],
        });
        this.blockLag = new prom_client_1.Gauge({
            name: 'arb_block_lag',
            help: 'Number of blocks behind the latest',
            registers: [this.register],
        });
        this.pendingTransactions = new prom_client_1.Gauge({
        ...
        // Histograms
        this.tradeProfitHistogram = new prom_client_1.Histogram({
        ...
        this.executionTimeHistogram = new prom_client_1.Histogram({
        ...
        this.gasUsedHistogram = new prom_client_1.Histogram({
            name: 'arb_gas_used',
        ...
        this.priceImpactHistogram = new prom_client_1.Histogram({
        ...
        this.simulationTimeHistogram = new prom_client_1.Histogram({
     ...
     */
    start(port) {
        const metricsPort = port || config_1.Config.monitoring.prometheusPort;
     ...
     */
    recordOpportunity(type, dex) {
        this.opportunitiesFound.inc({ type, dex });
    }
    /**
     * Record trade execution
     */
    recordTrade(type, dex, success, profitUsd, gasUsed, executionTimeMs) {
     ...
     */
    updateWalletBalance(token, balance) {
     ...
     * Update exposure
     */
    updateExposure(token, exposureUsd) {
        this.exposureByToken.set({ token }, exposureUsd);
    }
    /**
     * Update circuit breaker status
     */
    updateCircuitBreaker(active) {
     ...
     * Update risk score
     */
    updateRiskScore(type, score) {
        this.riskScore.set({ type }, score);
    }
    /**
     * Record price impact
     */
    recordPriceImpact(dex, impact) {
        this.priceImpactHistogram.observe({ dex }, impact);
    }
    /**
     * Record simulation time
     */
    recordSimulationTime(timeMs) {

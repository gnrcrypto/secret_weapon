...
export class Pathfinder {
   ...
   */
  async debugPathfinder(): Promise<void> {
    ...
    try {
      ...
      adapters.forEach((adapter, name) => {
        ...
        const protocol = (adapter as any).protocol || 'unknown';
  ...
  async findCrossDexPaths(
    tokenA: string,
    tokenB: string,
    maxPaths: number = 10  // Increased from 5
  ): Promise<ArbitragePath[]> {
    if (!this.initialized) await this.initialize();
      ...
      for (let i = 0; i < dexNames.length; i++) {
        for (let j = i + 1; j < dexNames.length; j++) {
          ...
          if (buyPairs.length > 0 && sellPairs.length > 0) {
            ...
            const forwardPath: ArbitragePath = {
  ...
  private getTokenSymbol(address: string): string {
    const token = this.commonTokens.find(t =>
   ...
   */
  async enumeratePaths(_maxHops: number = 3): Promise<ArbitragePath[]> {
    if (!this.initialized) await this.initialize();
